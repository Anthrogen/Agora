...
class Consensus(nn.Module):
    """
    Consensus Layer - Unrolled Learnable Consensus with Configurable Connectivity
    ...
    • Learnable step sizes eta^(t) for each iteration
    ...
    def __init__(
        ...
        if self.r > 0:
            # Lambda_ij matrix per edge (rank × dim)
            self.edge_to_Lambda = nn.Sequential(
                nn.Linear(2 * dim, edge_hidden_dim),
                nn.GELU(),
                nn.Linear(edge_hidden_dim, r * dim)
            )
        
        # Learnable step sizes η(t,i) for each iteration and residue
    ...
    def compute_edge_params(self, x, edge_i, edge_j):
        ...
        if self.r > 0:
            Lambda_flat = self.edge_to_Lambda(edge_features)  # [B, E, r*D]
            Lambda = Lambda_flat.view(B, -1, self.r, self.dim)  # [B, E, r, D]
            # Normalize U columns
    ...
    def consensus_step(self, u, edge_i, edge_j, alphas, step_sizes, Lambda=None):
        ...
        u_i = u.gather(1, edge_i.unsqueeze(-1).expand(-1, -1, D))  # [B, E, D]
        u_j = u.gather(1, edge_j.unsqueeze(-1).expand(-1, -1, D))  # [B, E, D]
        ...
        if self.r > 0:
            Lambda_diff = torch.einsum('berd,bed->ber', Lambda, diff)  # [B, E, r]
            residual = alphas.unsqueeze(-1) * diff + torch.einsum('ber,berd->bed', Lambda_diff, Lambda)  # [B, E, D]
        else:
            residual = alphas.unsqueeze(-1) * diff
        
        # Accumulate updates at vertices
        vertex_updates = torch.zeros_like(u)
        vertex_updates.scatter_add_(1, edge_i.unsqueeze(-1).expand(-1, -1, D), residual)
        vertex_updates.scatter_add_(1, edge_j.unsqueeze(-1).expand(-1, -1, D), -residual)
        
        # Consensus update with per-residue step sizes
        # step_sizes is [B, L, 1], vertex_updates is [B, L, D]
