...
def main():
    ...
    for iteration in range(train_cfg.num_iter):
        ...
        # Load encoder weights and freeze for stage 2
            ...
            # For stage 2, just create the single model (no sync needed since encoder is frozen)
            model = create_model_with_config(train_cfg.model_type, model_cfg, device)

            # Construct encoder checkpoint path based on model type and iteration
            encoder_checkpoint_path = Path(train_cfg.checkpoint_dir) / f"{train_cfg.model_type}_stage_1_iter{iteration+1}_{train_cfg.masking_strategy}.pt"
            ...
            optimizer = AdamW(model.decoder.parameters(), lr=train_cfg.learning_rate)
        ...
        for epoch in range(train_cfg.max_epochs):
            ...
            train_metrics_sum = {'loss': 0.0, 'rmsd': 0.0}
            num_batches = 0
            ...
            # -------------------- Validation -------------------- #
            val_metrics_sum = {'loss': 0.0, 'rmsd': 0.0}
            val_num_batches = 0
            ...
            history['val_rmsd'].append(val_metrics_sum['rmsd'])
            
            # Store in global metrics arrays
            all_metrics['val_loss'][iteration, epoch] = val_metrics_sum['loss']
            all_metrics['val_rmsd'][iteration, epoch] = val_metrics_sum['rmsd']
    ...
    loss_csv_path = Path(train_cfg.checkpoint_dir) / f"{train_cfg.model_type}_{model_cfg.stage}_val_loss_{train_cfg.masking_strategy}.csv"
    np.savetxt(loss_csv_path, all_metrics['val_loss'], delimiter=',', 
               header=f"Validation squared RMSD losses for {train_cfg.model_type} {model_cfg.stage}\nRows: iterations ({train_cfg.num_iter}), Columns: epochs ({train_cfg.max_epochs})",
               comments='# ')
    
    # Save validation RMSDs
    rmsd_csv_path = Path(train_cfg.checkpoint_dir) / f"{train_cfg.model_type}_{model_cfg.stage}_val_rmsd_{train_cfg.masking_strategy}.csv"
    np.savetxt(rmsd_csv_path, all_metrics['val_rmsd'], delimiter=',',
               header=f"Validation RMSDs for {train_cfg.model_type} {model_cfg.stage}\nRows: iterations ({train_cfg.num_iter}), Columns: epochs ({train_cfg.max_epochs})",
               comments='# ')
    ...
    print(f"    Mean final epoch: {all_metrics['val_rmsd'][:, -1].mean():.4f} ± {all_metrics['val_rmsd'][:, -1].std():.4f}")
    print(f"    Best single run: {all_metrics['val_rmsd'].min():.4f}")
