...
def create_linear_decay(base_learning_rate: float, min_learning_rate: float, num_steps_decay: int, num_steps_warmup: int):
    def lr_lambda(current_step): # One step = one forward pass through the model
        if current_step < num_steps_warmup: # Linear warmup from 0 to base_lr
            warmup_lr = (current_step / num_steps_warmup) * base_learning_rate
            return warmup_lr / base_learning_rate
        elif current_step < num_steps_decay: # Linear decay from base_lr to min_lr
            # Calculate decay progress after warmup period
            decay_start = num_steps_warmup
            decay_progress = (current_step - decay_start) / (num_steps_decay - decay_start)
