...
class SelfConsensus(nn.Module):
    ...
    def get_scored_window_connectivity(self, x, B, L, device, mask=None):
        ...
        mask_self = torch.eye(L, device=device, dtype=torch.bool)
        scores.masked_fill_(mask_self, float('-inf'))
        
        # Apply validity masking if provided
        if mask is not None:
            # Mask out connections involving invalid positions
            # If either source or target is invalid, mask the connection
            valid_mask = mask.unsqueeze(1) & mask.unsqueeze(2)  # [B, L, L]
        ...
        # Create edge validity mask: E_masked = {(i,j) ∈ E | m_i = 1 ∧ m_j = 1}
        if mask is not None: edge_valid = mask.gather(1, edge_i) & mask.gather(1, edge_j)  # [B, L*w]
        else: edge_valid = None
        
        return edge_i, edge_j, edge_valid
    
    def get_local_window_edges(self, B, L, device, mask=None):
        ...
        # Create edge validity mask: E_masked = {(i,j) ∈ E | m_i = 1 ∧ m_j = 1}
        if mask is not None: edge_valid = mask.gather(1, edge_i) & mask.gather(1, edge_j)  # [B, num_edges]
        else: edge_valid = None
        
        return edge_i, edge_j, edge_valid
    
    def get_connectivity(self, x, B, L, device, mask=None):
        """Get connectivity pattern based on configured type."""
        if self.connectivity_type == "scored_window":
            return self.get_scored_window_connectivity(x, B, L, device, mask)
        elif self.connectivity_type == "local_window":
            return self.get_local_window_edges(B, L, device, mask)
        else:
    ...
    def consensus_step(self, u, edge_i, edge_j, alphas, step_sizes, Lambda=None, edge_valid=None):
    ...
    def forward(self, x, mask=None):
        ...
        Args:
            ...
            mask: [B, L] boolean tensor where True = valid, False = invalid/padding
        ...
        # Get connectivity pattern with edge filtering based on mask
        # E_masked = {(i,j) ∈ E | m_i = 1 ∧ m_j = 1}
        edge_i, edge_j, edge_valid = self.get_connectivity(x, B, L, device, mask)
        
        # Compute edge parameters (only for valid edges)
        if self.r > 0:
            alphas, Lambda = self.compute_edge_params(x, edge_i, edge_j)
        
            # Unrolled consensus gradient updates on filtered edge set
            for t in range(self.num_iterations):
                ...
                u = self.consensus_step(u, edge_i, edge_j, alphas, step_sizes_t, Lambda, edge_valid)
            ...
            for t in range(self.num_iterations):
                ...
                step_sizes_t = F.softplus(self.step_sizes[t].unsqueeze(0).unsqueeze(0).unsqueeze(-1))  # [1, 1, L, 1]
                step_sizes_t = step_sizes_t.expand(B, self.heads, -1, -1)  # [B, heads, L, 1]
                u = self.consensus_step(u, edge_i, edge_j, alphas, step_sizes_t, None, edge_valid)
