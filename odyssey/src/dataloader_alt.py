...
class MaskedBatch():
    def __init__(self, data, masks, metadata, tracks, sequence_tokenizer, structure_tokenizer, coordinates_tokenizer, ss8_tokenizer, sasa_tokenizer, orthologous_groups_tokenizer,
        ...
        self.coordinates_tokenizer = coordinates_tokenizer
        self.ss8_tokenizer = ss8_tokenizer
        self.sasa_tokenizer = sasa_tokenizer
        self.orthologous_groups_tokenizer = orthologous_groups_tokenizer
        self.semantic_description_tokenizer = semantic_description_tokenizer
        self.domains_tokenizer = domains_tokenizer
        self.plddt_tokenizer = plddt_tokenizer
    ...
    def cpu_tok(self):
        ...
        seq_list, self.coords_list, ss8_list, sasa_list, orthologous_groups_list, semantic_description_list, domains_list, plddt_list, _ = zip(*self.data)
        # Stack lengths
        self.B = len(seq_list)
        self.L = self.sequence_tokenizer.full_length
        self.G = self.orthologous_groups_tokenizer.full_length
        self.H = self.semantic_description_tokenizer.full_length

        if self.tracks['seq']:
            seq_data = []
            for idx, seq in enumerate(seq_list):
                seq_data.append(self.sequence_tokenizer.tokenize(seq, self.masks['seq'][idx]))
            self.unmasked_data['seq'], self.masked_data['seq'], self.beospank['seq'], self.masks['seq'] = zip(*seq_data)
            self.unmasked_data['seq'] = torch.stack(self.unmasked_data['seq'], dim=0)
            self.masked_data['seq'] = torch.stack(self.masked_data['seq'], dim=0)
            self.beospank['seq'] = torch.stack(self.beospank['seq'], dim=0).bool()
            self.masks['seq'] = torch.stack(self.masks['seq'], dim=0).bool()
        ...
        if self.tracks['coords'] and not self.tracks['struct']:
            ...
            # Unpack the 4-tuple returned by tokenize_from_coords
            coords_results = list(zip(*coords_data))
            self.unmasked_data['coords'] = torch.stack(coords_results[0], dim=0)
            self.masked_data['coords'] = torch.stack(coords_results[1], dim=0)
            self.beospank['coords'] = torch.stack(coords_results[2], dim=0).bool()
            self.masks['coords'] = torch.stack(coords_results[3], dim=0).bool()

        # Tokenize SS8 (secondary structure) data if enabled
        if self.tracks['ss8']:
            ss8_data = []
            for idx, ss8 in enumerate(ss8_list):
                ss8_data.append(self.ss8_tokenizer.tokenize(ss8, self.masks['ss8'][idx]))
            self.unmasked_data['ss8'], self.masked_data['ss8'], self.beospank['ss8'], self.masks['ss8'] = zip(*ss8_data)
            self.unmasked_data['ss8'] = torch.stack(self.unmasked_data['ss8'], dim=0)
            self.masked_data['ss8'] = torch.stack(self.masked_data['ss8'], dim=0)
            self.beospank['ss8'] = torch.stack(self.beospank['ss8'], dim=0).bool()
            self.masks['ss8'] = torch.stack(self.masks['ss8'], dim=0).bool()

        # Tokenize SASA (solvent accessible surface area) data if enabled
        if self.tracks['sasa']:
            sasa_data = []
            for idx, sasa in enumerate(sasa_list):
                sasa_data.append(self.sasa_tokenizer.tokenize(sasa, self.masks['sasa'][idx]))
            self.unmasked_data['sasa'], self.masked_data['sasa'], self.beospank['sasa'], self.masks['sasa'] = zip(*sasa_data)
            self.unmasked_data['sasa'] = torch.stack(self.unmasked_data['sasa'], dim=0)
            self.masked_data['sasa'] = torch.stack(self.masked_data['sasa'], dim=0)
            self.beospank['sasa'] = torch.stack(self.beospank['sasa'], dim=0).bool()
            self.masks['sasa'] = torch.stack(self.masks['sasa'], dim=0).bool()
        ...
        if self.tracks['orthologous_groups']:
            ...
            for orthologous_groups in orthologous_groups_list:
                orthologous_groups_data.append(self.orthologous_groups_tokenizer.tokenize(orthologous_groups))
            self.unmasked_data['orthologous_groups'], self.beospank['orthologous_groups'] = zip(*orthologous_groups_data)
            self.unmasked_data['orthologous_groups'] = torch.stack(self.unmasked_data['orthologous_groups'], dim=0)
            self.beospank['orthologous_groups'] = torch.stack(self.beospank['orthologous_groups'], dim=0).bool()
        ...
        if self.tracks['semantic_description']:
            ...
            for semantic_description in semantic_description_list:
                semantic_description_data.append(self.semantic_description_tokenizer.tokenize(semantic_description))
            self.unmasked_data['semantic_description'], self.beospank['semantic_description'] = zip(*semantic_description_data)
            self.unmasked_data['semantic_description'] = torch.stack(self.unmasked_data['semantic_description'], dim=0)
            self.beospank['semantic_description'] = torch.stack(self.beospank['semantic_description'], dim=0).bool()

        # Tokenize domains data if enabled
        if self.tracks['domains']:
            domains_data = []
            for idx, domains in enumerate(domains_list):
                domains_data.append(self.domains_tokenizer.tokenize(domains, self.masks['domains'][idx]))
            self.unmasked_data['domains'], self.masked_data['domains'], self.beospank['domains'], self.masks['domains'] = zip(*domains_data)
            self.unmasked_data['domains'] = torch.stack(self.unmasked_data['domains'], dim=0)
            self.masked_data['domains'] = torch.stack(self.masked_data['domains'], dim=0)
            self.beospank['domains'] = torch.stack(self.beospank['domains'], dim=0).bool()
            self.masks['domains'] = torch.stack(self.masks['domains'], dim=0).bool()

        # Tokenize pLDDT data if enabled
        if self.tracks['plddt']:
            plddt_data = []
            for idx, plddt in enumerate(plddt_list):
                plddt_data.append(self.plddt_tokenizer.tokenize(plddt, self.masks['plddt'][idx]))
            self.unmasked_data['plddt'], self.masked_data['plddt'], self.beospank['plddt'], self.masks['plddt'] = zip(*plddt_data)
            self.unmasked_data['plddt'] = torch.stack(self.unmasked_data['plddt'], dim=0)
            self.masked_data['plddt'] = torch.stack(self.masked_data['plddt'], dim=0)
            self.beospank['plddt'] = torch.stack(self.beospank['plddt'], dim=0).bool()
    ...
    def gpu_tok(self):
        ...
        if self.tracks['struct']:
            ...
            self.beospank['coords'] = torch.stack(coords_results[2], dim=0).to(gpu).bool()
            self.masks['coords'] = torch.stack(coords_results[3], dim=0).to(gpu).bool()

            self.unmasked_data['struct'] = torch.stack(coords_results[4], dim=0).to(gpu)
