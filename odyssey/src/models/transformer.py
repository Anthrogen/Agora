...
class TransformerTrunk(nn.Module):
    ...
    def forward(
        ...
        mask_global_annotation: Optional[torch.Tensor] = None,  # [B, G] boolean mask
        mask_per_residue_annotation: Optional[torch.Tensor] = None,  # [B, L, K] boolean mask
        mask_plddt: Optional[torch.Tensor] = None,  # [B, L] boolean mask
        ...
        Args:
            ...
            mask_global_annotation: Optional boolean mask ``[B, G]`` indicating which positions
               have valid global annotation tokens. True = valid, False = masked.
            mask_per_residue_annotation: Optional boolean mask ``[B, L, K]`` indicating which positions
               have valid per-residue annotation tokens. True = valid, False = masked.
            mask_plddt: Optional boolean mask ``[B, L]`` indicating which positions
        ...
        valid_per_residue_annotations = (~mask_per_residue_annotation).unsqueeze(-1)  # [B, L, K, 1] True = valid annotation, False = special token
        ...
        # Only apply context injection to rows that have at least one valid context position
        valid_global_annotation_indices = mask_global_annotation.any(dim=1).nonzero(as_tuple=True)[0]  # Get indices of valid rows
        if len(valid_global_annotation_indices) > 0:
            global_annotation_context = self.context_global_annotation(h[valid_global_annotation_indices], global_annotation_emb[valid_global_annotation_indices], mask[valid_global_annotation_indices], mask_global_annotation[valid_global_annotation_indices])
        ...
        mask_per_residue_annotation_simplified = (~mask_per_residue_annotation).any(dim=2)  # [B, L]
        valid_per_residue_annotation_indices = mask_per_residue_annotation_simplified.any(dim=1).nonzero(as_tuple=True)[0]  # Get indices of valid rows
        if len(valid_per_residue_annotation_indices) > 0:
            per_residue_annotation_context = self.context_per_residue_annotation(h[valid_per_residue_annotation_indices], per_residue_annotation_emb[valid_per_residue_annotation_indices], mask[valid_per_residue_annotation_indices], mask_per_residue_annotation_simplified[valid_per_residue_annotation_indices])
        ...
        # Only apply context injection to rows that have at least one valid context position
        valid_plddt_indices = mask_plddt.any(dim=1).nonzero(as_tuple=True)[0]  # Get indices of valid rows
        if len(valid_plddt_indices) > 0:
            plddt_context = self.context_plddt(h[valid_plddt_indices], plddt_emb[valid_plddt_indices], mask[valid_plddt_indices], mask_plddt[valid_plddt_indices])
