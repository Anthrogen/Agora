...
from src.dataloader_trunk import MLMDataLoader, get_noise_levels
...
def evaluate_mlm_model(model: TransformerTrunk, dataloader: DataLoader, model_type: str, device: torch.device) -> Dict[str, float]:
    ...
    with torch.no_grad():
        for batch_data in dataloader:
            ...
            coord_mask = valid_mask & (~mask_struct)
            
            # Handle all-masked sequences
            all_masked = ~coord_mask.any(dim=1)
            if all_masked.any():
                for idx in torch.where(all_masked)[0]:
                    masked_coords[idx, 1, 0, :] = torch.tensor([0., 0., 0.], device=device)
                    masked_coords[idx, 1, 1, :] = torch.tensor([1.5, 0., 0.], device=device)
                    masked_coords[idx, 1, 2, :] = torch.tensor([2.4, 1.3, 0.], device=device)
                    coord_mask[idx, 1] = True
            
            # Forward pass
            if model_type in ("GA", "RA"):
                outputs = model((masked_seq, masked_struct), masked_coords, coord_mask)
            else:
                outputs = model((masked_seq, masked_struct))
            ...
            seq_loss = F.cross_entropy(
                seq_logits_flat[coord_mask.reshape(-1)],
                seq_labels_flat[coord_mask.reshape(-1)].long(),
                reduction='mean'
            )
            
            struct_loss = F.cross_entropy(
                struct_logits_flat[coord_mask.reshape(-1)],
                struct_labels_flat[coord_mask.reshape(-1)].long(),
                reduction='mean'
            )
...
def evaluate_diffusion_model(model: TransformerTrunk, batch_data: Tuple, timestep: int, 
    ...
    # We need to prepare the batch data in the format expected by diffusion models
    # The MLM dataloader gives us: (inputs, masks, lengths, original_tokens)
    inputs, masks, lengths, original_tokens = batch_data
    masked_seq, masked_struct, masked_coords = inputs
    mask_seq, mask_struct = masks
    seq_tokens, struct_tokens = original_tokens
    ...
    # Use the noisy inputs as x_t and original tokens as x_0
    seq_x_t = masked_seq
    struct_x_t = masked_struct
    seq_x_0 = seq_tokens
    struct_x_0 = struct_tokens
    ...
    with torch.no_grad():
        # Forward pass
        if model_type in ("GA", "RA"):
            outputs = model((seq_x_t, struct_x_t), masked_coords, coord_mask, timesteps)
        else:
...
def main(custom_config: Optional[ValidationConfig] = None):
    ...
    dataset = ProteinBackboneDataset(val_cfg.data_dir, max_length=mlm_model_cfg.max_len - 2)
    ...
    for t_idx in val_cfg.time_indices:
        ...
        for training_method in val_cfg.training_methods:
            ...
            # Create dataloader with fixed mask probability (same for all methods)
            val_loader = MLMDataLoader(
                dataset, fsq_encoder=fsq_encoder, model_cfg=model_cfg_for_loader,
            ...
            if training_method in ['simple', 'complex']:
                ...
                for batch_data in val_loader:
                    batch_metrics = evaluate_diffusion_model(
                        model, batch_data, t_idx, diffusion_cfg, diffusion_model_cfg, val_cfg.model_type, device
                    )
